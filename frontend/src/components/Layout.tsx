...
import { Settings, LayoutDashboard, ChevronLeft, Users, ChevronRight, Receipt, Calendar, Stethoscope, BarChart3 } from 'lucide-react';
...
export default function Layout() {
  ...
  const currentLabel = NAV.find(n => n.to === '/' ? location.pathname === '/' : location.pathname.startsWith(n.to))?.label || 'MediCare';
    ...
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      <aside className={cn('flex flex-col bg-primary text-white transition-all duration-200 shrink-0', collapsed ? 'w-16' : 'w-56')}>
        <div className={cn('flex items-center gap-2 border-b border-white/10 px-4 py-4', collapsed && 'justify-center px-2')}>
          ...
          {!collapsed && <span className="font-bold text-lg">MediCare</span>}
        </div>
